...
After installing, copy the *examples/nlpserver* file to */etc/apache2/sites-available*. Modify the user and group field, and also the paths to the app. Change the port from 8139 if you want, but make sure to add the line `Listen 8139` to */etc/apache2/ports.conf*, and that it is open in the firewall. To run the server over *https*, modify the SSLCertificate lines to point to the pem and key file for your server. You might have to enable SSL for apache, if it's not done already. (If you don't need the SSL option for your site, just comment out the lines with SSL)
